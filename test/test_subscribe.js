...
async function test_subscribe() {
  ...
  let Client = require("../src/streaming/Client");
  let myConnect = new DBconnection();
  let config = require("./setup/settings");
  let client = new Client(config.SUBPORT);
  ...
  describe("#subscribe.js", function () {
    describe("#subscribe()", function () {
      it("subscribe offset 0", async function () {
        ...
        client.subscribe(config.HOST, config.PORT, "trades", "trades_sub1", {
          handler: f1,
          offset: 0,
        });
      ...
      it("multiple subscription", async function () {
        ...
        client.subscribe(config.HOST, config.PORT, "trades", "trades_sub2", {
          handler: f2,
          offset: 0,
        });
      ...
      it("subscribe filter date", async function () {
        ...
        client.subscribe(config.HOST, config.PORT, "trades", "trades_sub1", {
          handler: f1,
          offset: 0,
          filter: DT.Date(["2012-01-01", "2012-01-05"]),
        });
        await myConnect.run("sleep(3000)");
      ...
      it("subscribe filter month", async function () {
        ...
        client.subscribe(config.HOST, config.PORT, "trades", "trades_sub1", {
          handler: f1,
          offset: 0,
          filter: DT.Month(["2012-03", "2012-07"]),
        });
        await myConnect.run("sleep(3000)");
        assert.deepEqual(arr[0][0], [1, 5]);
        assert.equal(arr[0][1].toString(), "2012-1-1,2012-1-5");
      ...
      it("subscribe filter time", async function () {
        ...
        client.subscribe(config.HOST, config.PORT, "trades", "trades_sub1", {
          handler: f1,
          offset: 0,
          filter: DT.Time(["13:30:10.007", "13:30:10.009", "13:30:10.008"]),
        });
        await myConnect.run("sleep(3000)");
        ...
        assert.deepEqual(
          arr[0][3].toString(),
          "13:30:10.7000000ns,13:30:10.8000000ns,13:30:10.9000000ns"
        );
        await client.unsubscribe(
          config.HOST,
          config.PORT,
      ...
      it("subscribe filter timestamp", async function () {
        ...
        client.subscribe(config.HOST, config.PORT, "trades", "trades_sub1", {
          ...
          filter: DT.TimeStamp([
            ...
            "2012-06-13 13:30:10.009",
            "2012-06-13 13:30:10.012",
          ]),
        });
        await myConnect.run("sleep(3000)");
        ...
        assert.deepEqual(
          arr[0][3].toString(),
          "2012-6-13 13:30:10.8000000ns,2012-6-13 13:30:10.9000000ns,2012-6-13 13:30:10.12000000ns"
        );
        await client.unsubscribe(
          config.HOST,
          config.PORT,
      ...
      it("subscribe filter nanotimestamp", async function () {
        ...
        client.subscribe(config.HOST, config.PORT, "trades", "trades_sub1", {
          handler: f1,
          offset: 0,
          filter: DT.NanoTimeStamp([
            "2012-6-13 13:30:10.008007006",
            "2012-6-13 13:30:10.100",
          ]),
        });
        await myConnect.run("sleep(3000)");
        assert.deepEqual(arr[0][0], 1);
