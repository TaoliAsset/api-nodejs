...
class BasicNull extends BasicScalar {
  constructor(dtype) {
    super();
    if (typeof dtype === "string") this.dtype = Util.dtypeByName(dtype);
    else {
      this.dtype = dtype;
    }
  ...
  nullTypeValue() {
    switch (this.dtype) {
      // case -1: return 0;
      case 0:
        return nullV; //VOID, value not sure
      case 1:
        return byteMin; //BOOL
      case 2:
        return byteMin; //CHAR
      case 3:
        return shortMin; //SHORT
      case 4:
        return intMin; //INT
      case 5:
        return longMin; //LONG
      ...
      case 8:
        return intMin; //TIME
      case 9:
        return intMin; //MINUTE
      case 10:
        return intMin; //SECOND
      case 11:
        return intMin; //DATETIME
      case 12:
        return longMin; //TIMESTAMP
      case 13:
        return longMin; //NANOTIME
      case 14:
        return longMin; //NANOTIMESTAMP
      case 15:
        return floatMin; //FLOAT
      case 16:
        return doubleMin; //DOUBLE
      default:
        return null;
    }
  }
  ...
  tobytes() {
    if (this.dtype === 0) return Buffer.from([this.value]);
    if (this.dtype === 1 || this.dtype === 2)
      return new BasicByte(byteMin).tobytes();
    else if (this.dtype === 3) return new BasicShort(shortMin).tobytes();
