...
function assertLog(check, name) {
  if (check) console.log(name + " pass\n");
...
async function main() {
  ...
  if (test) {
    ...
    assertEquals(
      "2013-6-13",
      (await myConnect.run("2013.06.13")).toString(),
      "date"
    );
    assertEquals(
      "2012-6",
      (await myConnect.run("2012.06M")).toString(),
      "month"
    );
    assertEquals(
      "13:30:10.8000000ns",
      (await myConnect.run("13:30:10.008")).toString(),
      "time"
    );
    assertEquals("13:30", (await myConnect.run("13:30m")).toString(), "minute");
    assertEquals(
      "13:30:10",
      (await myConnect.run("13:30:10")).toString(),
      "second"
    );
    assertEquals(
      "2012-6-13 13:30:10",
      (await myConnect.run("2012.06.13 13:30:10")).toString(),
      "datetime1"
    );
    assertEquals(
      "1960-6-13 13:30:10",
      (await myConnect.run("1960.06.13 13:30:10")).toString(),
      "datetime2"
    );
    assertEquals(
      "2012-6-13 13:30:10.8000000ns",
      (await myConnect.run("2012.06.13 13:30:10.008")).toString(),
      "timestamp1"
    );
    assertEquals(
      "1960-6-13 13:30:10.8000000ns",
      (await myConnect.run("1960.06.13 13:30:10.008")).toString(),
      "timestamp2"
    );
    assertEquals(
      "13:30:10.8007006ns",
      (await myConnect.run("13:30:10.008007006")).toString(),
      "nanotime"
    );
    assertEquals(
      "2012-6-13 13:30:10.8007006ns",
      (await myConnect.run("2012.06.13 13:30:10.008007006")).toString(),
      "nanotimestamp1"
    );
    assertEquals(
      "1958-6-13 13:30:10.8007006ns",
      (await myConnect.run("1958.06.13 13:30:10.008007006")).toString(),
      "nanotimestamp2"
    );
    result = await myConnect.run(
    ...
    assertLog(
      assertArrayEquals(null, result.rownames) &&
        assertArrayEquals(null, result.colnames) &&
        assertArrayEquals(
          [
            [1, 2],
            [3, 4],
    ...
    assertLog(
      assertArrayEquals(null, result.rownames) &&
        assertArrayEquals(["c1", "c2", "c3", "c4"], result.colnames) &&
        assertArrayEquals(
          [
            [1, 2, 3],
            [4, 5, 6],
    ...
    assertLog(
      assertArrayEquals(["r1", "r2", "r3"], result.rownames) &&
        assertArrayEquals(null, result.colnames) &&
        assertArrayEquals(
          [
            [1, 2, 3],
    ...
    result = await myConnect.run(
      "m=1..12$3:4;m.rename!(`r1`r2`r3,`c1`c2`c3`c4);m;"
    );
    assertLog(
      assertArrayEquals(["r1", "r2", "r3"], result.rownames) &&
        assertArrayEquals(["c1", "c2", "c3", "c4"], result.colnames) &&
    ...
    // console.log(result)
    assertLog(
      assertArrayEquals(["aa", "bb", "rr"], Array.from(result).sort()),
      "set2"
    );
    //dictionary
    ...
    assertLog(
      assertArrayEquals(
        ["AA", "BB", "CC"],
        [result.get(1), result.get(7), result.get(5)]
      ),
      "dict1"
    );
    result = await myConnect.run(
      "dict(`IBM`GOOG`MSFT, (1 2 3, 4 5 6, 7 8 9));"
    );
    // console.log(result)
    assertLog(
      assertArrayEquals(
        ...
        [result.get("IBM"), result.get("GOOG"), result.get("MSFT")]
      ),
      "dict2"
    );
    // table
    console.log("table");
    result = await myConnect.run(
      "t1=table(1 2 3 as a, `x`y`z as b, 10.8 7.6 3.5 as c);t1;"
    );
    assertLog(
      result.tablename === "t1" &&
        assertArrayEquals(["a", "b", "c"], result.colnames) &&
        assertArrayEquals(
          [
            [1, 2, 3],
            ["x", "y", "z"],
    ...
    assertLog(
      result.tablename === "" &&
        assertArrayEquals(["col0", "col1", "col2"], result.colnames) &&
        assertArrayEquals(
          [
            [1, 2, 3],
            [4, 5, 6],
  ...
  if (debug) {
    ...
    client.subscribe("192.168.1.111", 8848, "trades3", "trades_sub1", {
      handler: handleMsg,
      offset: -1,
      filter: null,
    });
